...
tf.app.flags.DEFINE_string("openpose", "output_json/tmp", "Data directory")
tf.app.flags.DEFINE_integer("verbose", 2, "Set logging level")
tf.app.flags.DEFINE_integer("person_idx", 1, "The number of person in video")
tf.app.flags.DEFINE_integer("gif_fps", 10, "Set FPS of output gif image.")
...
def extraction_serialnum(filename):  # ファイルパスlist.sort()で使う
    return int(filename.lstrip(FLAGS.openpose+"/").rstrip("_keypoints.json"))
...
def read_openpose_json(smooth=False, *args):
    ...
    json_files.sort(key=extraction_serialnum)
    cache = {}
    smoothed = {}
